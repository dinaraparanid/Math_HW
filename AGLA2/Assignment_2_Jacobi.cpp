...
namespace agla {
	namespace mtx {
		...
		template <typename T> [[nodiscard]] inline bool is_jacobi_applicable(const matrix<T>& mtx) noexcept {
			return std::all_of(
					mtx.rows_begin(),
					mtx.rows_end(),
...
int main() {
	...
	if (!agla::mtx::is_jacobi_applicable(a)) {
	...
	for (;;) {
		...
		auto next = agla::column_vec::column_vector(
				(x + agla::column_vec::column_vector(
						(da_inv *
						(b - agla::column_vec::column_vector(
								(a * x).value()
								))
								.value())
						.value()))
