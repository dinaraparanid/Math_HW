...
int main() {
	...
	agla::column_vec::column_vector<double> b(m);
	std::vector<double> a_buf(m);

	for (std::size_t i = 0; i < m; ++i) {
		double ti = 0, bi = 0;
		std::scanf("%lf%lf", &ti, &bi);
	...
	const auto at_a = agla::mtx::square_matrix((at * a).value());
	std::puts("A_T*A:");
	std::cout << at_a;

	const auto at_a_inv = at_a.inversed().value();
	std::puts("(A_T*A)^-1:");
	std::cout << at_a_inv;

	const auto at_b = (at * b).value();
	std::puts("A_T*b:");
	std::cout << at_b;

	const auto x = (at_a_inv * at_b).value();
	std::puts("x~:");
	std::cout << x;
